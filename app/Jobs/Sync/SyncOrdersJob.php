...
class SyncOrdersJob implements ShouldQueue
{
    ...
    public function handle(NuvemshopService $nuvemshopService): void
    {
        ...
        if ($user && $planLimitService->hasExceededOrdersLimit($user)) {
            Log::channel('sync')->info('>>> [PARALLEL] Sync de PEDIDOS ignorado - limite do plano atingido', [
        ...
        Log::channel('sync')->info('>>> [PARALLEL] Iniciando sync de PEDIDOS', [
            'store_id' => $this->store->id,
            'store_name' => $this->store->name,
        ...
        try {
            $nuvemshopService->syncOrders($this->store, $this->updatedSince);

            $elapsed = round((microtime(true) - $startTime) * 1000, 2);
            Log::channel('sync')->info('<<< [PARALLEL] Sync de PEDIDOS concluido', [
                'store_id' => $this->store->id,
                'time_ms' => $elapsed,
                'orders_count' => $this->store->orders()->count(),
            ]);
        } catch (\Exception $e) {
            Log::channel('sync')->error('!!! [PARALLEL] Erro no sync de PEDIDOS', [
                'store_id' => $this->store->id,
                'error' => $e->getMessage(),
            ]);
            throw $e;
