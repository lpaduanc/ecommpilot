...
class SyncCustomersJob implements ShouldQueue
{
    ...
    public function handle(NuvemshopService $nuvemshopService): void
    {
        ...
        if ($this->store->token_requires_reconnection) {
            Log::channel('sync')->warning('>>> [CUSTOMERS] Sync ignorado - loja requer reconexão OAuth', [
        ...
        Log::channel('sync')->info('>>> [PARALLEL] Iniciando sync de CLIENTES', [
            'store_id' => $this->store->id,
            'store_name' => $this->store->name,
        ...
        try {
            ...
            Log::channel('sync')->error('!!! [PARALLEL] Erro no sync de CLIENTES', [
                'store_id' => $this->store->id,
                'error' => $e->getMessage(),
            ]);
            throw $e;
