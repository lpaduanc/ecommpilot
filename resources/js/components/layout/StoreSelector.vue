    ...
    <div v-if="stores.length > 0" class="store-selector relative">
        ...
        >
            ...
            <div class="p-2 border-t border-gray-100">
                <button
                    @click="syncStore"
                    :disabled="isSyncing || ['syncing', 'pending'].includes(activeStore?.sync_status)"
                ...
                >
                    <ArrowPathIcon :class="['w-4 h-4', { 'animate-spin': isSyncing || ['syncing', 'pending'].includes(activeStore?.sync_status) }]" />
                    {{ isSyncing || ['syncing', 'pending'].includes(activeStore?.sync_status) ? 'Sincronizando...' : 'Sincronizar Agora' }}
                </button>
            </div>
        </div>
    </div>
